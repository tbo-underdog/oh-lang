#!/bin/bash
# TLS 1.3 client: completes a handshake with openssl s_server -www (Ed25519 cert),
# verifies the server's CertificateVerify signature, and decrypts the HTTP response.
...
SRC="std/core.oh std/net.oh std/x25519.oh std/sha256.oh std/sha512.oh std/hkdf.oh std/tls13.oh std/aes.oh std/gcm.oh std/ed25519.oh projects/19_tls/tls_client.oh"
if [ "$ARCH" = arm64 ]; then
...
echo "$out"|grep -q "certificate verified" && echo "$out"|grep -q "server Finished verified" && echo "$out"|grep -q "HTTP/1.0 200" \
  && echo "TLS 1.3 OK (arch=$ARCH): cert verified + handshake + decrypted HTTP" || { echo "FAIL:"; echo "$out"|head -3; exit 1; }
